...
int ResCache::Preload(ResCache* pResCache, const std::string pattern, void(*progressCallback)(int, bool &))
{
	if (m_file == NULL)
		return 0;
	...
	for (int i = 0; i < numFiles; ++i)
	{
		...
		{
			shared_ptr<ResHandle> handle = pResCache->GetHandle(&resource);
			++loaded;
		}
